...
var color = d3.scaleSqrt().domain([2, 20]).range(d3.schemeYlOrRd[9]);

function getColor(scheme) {
  return d3.scaleLinear().domain([0, 3]).range(scheme);
}
...
Promise.all([
  ...
  d3.json("data/stateCounts.json"),
  d3.json("data/cityCounts.json"),
  d3.json("data/cityVictims.json"),
...
const numPerRow = Math.floor(
  d3.select(".grid-g").attr("width") / (gridRadius * 2.5)
);
...
function populateGrid(data, all = true) {
  ...
  if (!all || !isLoaded) {
    ...
    g_grid
      ...
      .append("a")
      .attr("href", (d) => {
        if (Array.isArray(d)) return d[1]["url"];
        else return d["url"];
      })
      .attr("target", "_blank")
      .append("circle")
      ...
      .attr("fill", (d) => {
        let obj = null;
        if (Array.isArray(d)) obj = d[1];
        else obj = d;

        let { gender, ageGroup } = obj;
        let scheme = d3.schemeGreys[9];
        if (gender === "M") scheme = d3.schemeBlues[7];
        else if (gender === "F") scheme = d3.schemeRdPu[8];
        else scheme = d3.schemeGreys[9];
        let circlecolor = getColor(scheme);

        if (ageGroup === "3") return circlecolor(20);
        else if (ageGroup === "2") return circlecolor(13);
        else if (ageGroup === "1") return circlecolor(7);
        else return circlecolor(100);
      })
      .attr("stroke-width", 2)
      .attr("stroke", "white")
      .on("mousemove", function (d) {
        ...
        if (d3.event.layerX < map_w / 2) {
          d3.select("#gridtip-container").style(
            "left",
            d3.event.layerX + 15 + "px"
          );
        } else {
          let tooltip_w = $("#gridtip-container").width();
          d3.select("#gridtip-container").style(
            "left",
            d3.event.layerX - tooltip_w - 30 + "px"
          );
        }
        ...
        if (d3.event.layerY < map_h / 2) {
          d3.select("#gridtip-container").style(
            "top",
            d3.event.layerY + 15 + "px"
          );
        } else {
          let tooltip_h = $("#gridtip-container").height();
          d3.select("#gridtip-container").style(
            "top",
            d3.event.layerY - tooltip_h - 30 + "px"
          );
        }
      })
      .on("mouseout", function () {
    ...
    if (all) isLoaded = true;
  }

  if (data.length === 0) d3.select(".death-count").text("12070");
  else d3.select(".death-count").text(data.length);
...
const radius = d3.scaleSqrt().domain([0, 1e3]).range([0, 15]);

var simulation = null;

let gradient_g = svg.append("g").attr("class", "gradient");
...
function clicked(d) {
  ...
  d3.selectAll("span.location-text").text(d.properties.NAME);

  svg.selectAll(".us-state").classed("blur", true);
  ...
  if (active.node() !== this) {
    ...
    d3.selectAll(".grad").remove();

    d3.selectAll("defs").remove();

    active.classed("active", false);
    active = d3.select(this).classed("active", true).classed("blur", false);
    ...
    let source = svg
      ...
      .attr("fill", function (d, i) {
        ...
        if (genderMale === 0) return "red";
        else if (genderFemale === 0) return "#1E90FF";
        else if (genderMale === 0 && genderFemale === 0) return "black";
        else {
    ...
    simulation = d3
      ...
      .force(
        "gravity",
        d3.forceManyBody().strength(0.01).distanceMax(1000).distanceMin(2)
      )
    ...
    simulation.nodes(cityDeaths).on("tick", ticked);
...
function reset() {
  ...
  d3.selectAll("span.location-text").text("USA");
  ...
  svg.selectAll("g.links").selectAll("line").remove();
  ...
  svg.selectAll(".us-state").classed("blur", false);
